...
def load_results(ArchTable, surfs=None, surfs_bot=None, units=None, facies=None, props=None, verbose = 0):
...
def load_bh_files(list_bhs, facies_data, units_data,
                  ...
                  dic_units_names={},
                  dic_facies_names={}, altitude=True, verbose = 1):
...
def extract_bhs(df, list_bhs, ArchTable, units_to_ignore=(), facies_to_ignore=(), extract_units=True, extract_facies=True, ignore2None=True, fill_gaps=True, vb=0, updater = None, verbose=0):
    ...
    for idx in df.index:
        ...
        if updater is not None:
            updater.update_state(state='PROGRESS', meta={'step': 15 + (idx / nbore) * 5, 'total_steps': 100, 'step_status': "Boreholes Processing"})
        #store borehole
...
def import_project(project_name, ws, import_bhs=True, import_results=True, import_grid=True, verbose = True):
    ...
    with open(os.path.normpath(os.path.join(ws, project_name+".yaml")), "r") as f:
...
def save_project(ArchTable, results=True, verbose=0):
