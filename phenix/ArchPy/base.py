...
def load_geotiff_and_resample(origin, resolution, grid_size, file_paths):
    ox, oy = origin
    sx, sy = resolution
    nx, ny = grid_size
    ...
    with rasterio.open(file_paths) as src:
        # Determine the subset of the image that corresponds to the grid
        xmin = ox
        ymin = oy
        xmax = ox + sx * nx
        ymax = oy + sy * ny
        ...
        rasterio.warp.reproject(
            ...
            src_crs=src.crs,
            dst_transform=rasterio.transform.from_origin(ox, ymax, sx, sy),
            dst_crs=src.crs,
            resampling=Resampling.bilinear
        )

    return np.flip(resampled, 0)
...
class Arch_table():
    ...
    def add_grid(self, dimensions, spacing, origin, top=None, bot=None, rspl_method="nearest", polygon=None, mask=None):
        ...
        if isinstance(top, str) or isinstance(bot, str):
            ...
            if isinstance(top, str):
                ...
                top = load_geotiff_and_resample([ox, oy] , [sx, sy], [nx, ny], top)
    ...
    def compute_surf(self, nreal=1, fl_top=True, rm_res_files=True, updater = None):
        ...
        if self.check_units_ID() is None:  # check if units have correct IDs
            return None
        self.get_pile_master().compute_surf(self, nreal, fl_top, vb=self.verbose, updater=updater)  # compute surfs of the first pile
        if updater is not None:
            updater.update_state(state='PROGRESS', meta={'step': 90, 'total_steps': 100, 'step_status': "PostProcess Units Simulation"})
    ...
    def fill_top_unit(self, method = "nearest_neighbors"):
...
class Pile():
    ...
    def compute_surf(self, ArchTable, nreal=1, fl_top=False, subpile=False, tops=None, bots=None, vb=1, updater=None):
        ...
        for ireal in range(nreal): # loop over real
            if updater is not None:
                mov = ireal/nreal
                updater.update_state(state='PROGRESS', meta={'step': int(20 + mov*70), 'total_steps': 100, 'step_status': "Units Simulation"})
